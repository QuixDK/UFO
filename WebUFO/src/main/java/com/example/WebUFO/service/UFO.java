...
import com.example.WebUFO.model.User;
import com.example.WebUFO.model.UserRepository;
...
public class UFO extends TelegramLongPollingBot {
    ...
    private void sendMessage(long chatID, String text) {
        ...
        if (user.isPresent()) {
            if (user.get().getUserState() == 1) {
                ...
                try {
                    execute(setPayButtons(sendMessage, user.get().getPayUlr()));
                    log.info("Message: " + text + " send to " + chatID);
                } catch (Exception e) {
                    log.error("Error occurred: " + e.getMessage());
                }
    ...
    private void createBill(String textMessage, long chatID) {
        BigDecimal bigDecimal = new BigDecimal(Integer.parseInt(textMessage));
        Currency currency = Currency.getInstance("RUB");
        MoneyAmount amount = new MoneyAmount(bigDecimal, currency);
        String comment = String.valueOf(chatID);
        String billID  = String.valueOf(UUID.randomUUID());
        CreateBillInfo billInfo = new CreateBillInfo(billID, amount, comment, ZonedDateTime.now().plusHours(1),null,null);
        try {
            String payUrl = client.createBill(billInfo).getPayUrl();
            userRepository.findById(chatID).get().setPayStatus("IN_PROCESS");
            userRepository.findById(chatID).get().setPayUrl(payUrl);
            userRepository.findById(chatID).get().setUserState(7);
            userRepository.findById(chatID).get().setBillID(billID);
            executeDbUpdate(PaymentProcess,chatID);
            executeDbUpdate(SetPayURL + "\"" + payUrl + "\"" + " where chatid = ",chatID);
            executeDbUpdate(SetBillID + "\""+billID +"\""+ " where chatid = ",chatID);
            sendPaymentBill(chatID, payUrl);
        }
        catch (Exception e) {
